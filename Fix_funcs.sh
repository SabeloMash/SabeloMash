...
getHardDisk() {
    ...
    # Then remove USB/removable devices so deployment cannot target them.
    local raw_devs devs
    raw_devs=$(lsblk -dpno KNAME,SIZE -I 3,8,9,179,202,253,259 | awk '$2 != "0B" && !seen[$1]++ { print $1 }')
    devs=""
    for dev in $raw_devs; do
        ...
        # Skip USB-connected block devices.
        if [[ $dev_sysfs == *"/usb"* || $dev_sysfs == *"/usb/"* || $dev_sysfs == *"usb"* ]]; then
            [[ -n $isdebug ]] && echo "Skipping USB device candidate: $dev"
            continue
        fi

        # Skip removable media (RM=1).
        if [[ -f "/sys/block/$dev_name/removable" ]] && [[ "$(cat "/sys/block/$dev_name/removable")" -eq 1 ]]; then
            [[ -n $isdebug ]] && echo "Skipping removable device candidate: $dev"
            continue
        fi
